...
description: "The Healine MCP server connects Claude Desktop and Cursor to health data. Query sleep, workouts, and activity in natural language. FastMCP-based."
keywords: ["mcp server health data", "model context protocol wearables", "health data mcp", "wearable ai integration"]
"og:title": "Health Data MCP Server | Healine"
...
"og:type": "article"
"twitter:title": "Health Data MCP Server | Healine"
...
MCP (Model Context Protocol) is a standard for connecting AI assistants to external data sources. The Healine MCP server enables AI tools like **Claude Desktop** and **Cursor** to query your wearable health data through natural language conversations.
...
This demo uses generated sample data to show the capabilities of Claude's MCP integration with Healine. User 1 has short, irregular sleep patterns while User 2 shows longer, more consistent sleep quality. Claude analyzes both datasets to identify key differences and health implications.

<iframe
  width="100%"
  height="400"
  src="https://www.youtube.com/embed/Jb8N2jNbs1Y"
  title="Healine MCP Server Demo"
  frameBorder="0"
...
<Tip>
  To generate your API key, go to the **Healine Dashboard -> Settings -> Credentials -> Create API Key**.
</Tip>
...
```bash
2026-02-10 07:48:22,016 - app.main - INFO - Healine MCP server initialized. API URL: http://localhost:8000
[02/10/26 07:48:22] INFO     Starting MCP server 'healine' with transport 'stdio'
