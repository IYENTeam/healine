...
keywords: ["healine api reference", "wearable rest api docs", "health data api endpoints", "healine endpoints"]
"og:title": "Healine API Reference | REST API Docs"
...
"og:type": "article"
"twitter:title": "Healine API Reference | REST API Docs"
...
The Healine API provides a unified REST interface to access health data from multiple wearable devices and fitness platforms. All endpoints are versioned under `/api/v1` and follow RESTful conventions.
...
<Warning>
  The full key is displayed only once, right after you create or rotate it. Healine stores a hash of the key, so it cannot be retrieved later - the portal shows just a short prefix (for example `sk-a1b2c3d`) to help you tell keys apart. If you lose a key, rotate it from the same section to get a new one.
</Warning>
