...
keywords: ["wearable api integration guide", "healine quick start", "health api tutorial", "wearable data integration"]
"og:title": "Wearable API Integration Guide | Healine"
...
"og:type": "article"
"twitter:title": "Wearable API Integration Guide | Healine"
...
This guide walks you through the complete flow of integrating Healine with your application, from creating a user to fetching their health data.

```
1. Create a user in Healine
...
First, create a user in Healine to represent your application's user.
...
<Tip>
  Use `external_user_id` to map Healine users to your application's user IDs for easy lookup later.
</Tip>
...
<Warning>
  Healine allows duplicate email addresses. If you need uniqueness, check for existing users first using `GET /api/v1/users?search=user@example.com` which returns a paginated list.
</Warning>
