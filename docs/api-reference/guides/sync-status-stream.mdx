...
keywords: ["sync status", "server-sent events", "SSE", "real-time sync", "sync progress", "healine"]
"og:title": "Live Sync Status (SSE) | Healine"
"og:description": "Subscribe to real-time sync lifecycle events for any user via SSE."
"og:type": "article"
"twitter:title": "Live Sync Status (SSE) | Healine"
...
curl -H "X-Open-Wearables-API-Key: YOUR_API_KEY" \
  "http://localhost:8000/api/v1/users/USER_ID/sync/stream"
```
</CodeGroup>
...
```bash
curl -N "http://localhost:8000/api/v1/users/USER_ID/sync/stream?replay=20" \
...
const url =
  'http://localhost:8000/api/v1/users/USER_ID/sync/stream?replay=20';
...
```bash
curl "http://localhost:8000/api/v1/users/USER_ID/sync/recent?limit=50" \
  -H "X-Open-Wearables-API-Key: YOUR_API_KEY"
```
...
```bash
curl "http://localhost:8000/api/v1/users/USER_ID/sync/runs?limit=20" \
  -H "X-Open-Wearables-API-Key: YOUR_API_KEY"
```
