...
description: "Manage app-level webhook subscriptions with Polar, Oura, and Strava so those providers push events to your Healine instance in real time."
keywords: ["provider webhooks", "webhook subscriptions", "polar webhook", "oura webhook", "strava webhook", "incoming webhooks"]
"og:title": "Provider Webhook Subscriptions | Healine"
...
<Note>
  **Polar**: the `signature_secret_key` is returned by Polar exactly once, on creation. Healine saves it automatically to the `provider_settings` table (`webhook_secret` column). If you need to rotate it, delete and recreate the webhook.
</Note>
...
Withings recommends adding a random shared secret to the exact callback URL. Set `WITHINGS_WEBHOOK_TOKEN` to a value generated with `openssl rand -hex 32`; Healine registers `?token=<secret>` and validates it on notification POSTs and reachability HEAD probes. See the [Withings notification security guidance](https://developer.withings.com/developer-guide/v3/integration-guide/public-health-data-api/data-api/notifications/notification-overview/).
