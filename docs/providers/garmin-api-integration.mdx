...
"og:title": "Garmin API Integration | Healine"
...
"og:type": "article"
"twitter:title": "Garmin API Integration | Healine"
...
<Note>
**Garmin API giving you a headache?** We feel your pain - that's exactly why we created Healine. Pop into our [Discord](https://discord.gg/qrcfFnNE6H) if you have questions or want to discover how Healine can solve your problems.
</Note>
...
<Steps>
  ...
  <Step title="Configure API Tools (endpoints + data types/scopes)">
    ...
    **Enable endpoints**
    ...
    - For **every** endpoint, set the URL to your Healine webhook push endpoint:
    ...
    **Which endpoints to enable:**
    ...
    Enable **all** endpoints listed on the page. Healine accepts data from all Garmin data types (activities, dailies, sleeps, epochs, HRV, body compositions, stress, respiration, pulse ox, and more).
    ...
    **Backfill requirement**: backfill only works once the relevant endpoints are enabled on this page.
    ...
    Once backfill is enabled and data is ready, Garmin will deliver notifications via push and Healine can ingest the backfilled data.
    </Warning>
    ...
    **(Optional) Modify data types and OAuth scopes**
    ...
    <Note>
    Keep scopes minimal: only request what Healine actually needs for your use case. If you add new data domains later, you’ll typically need to update this configuration and re-verify.
    </Note>
  </Step>

  <Step title="Provide Garmin credentials to Healine">
    Once you have an app created, configure your Healine backend with:
