---
title: "HealthKit Integration with Healine"
description: "Integrate Apple Health via HealthKit using the Healine iOS, Flutter, and React Native SDKs. Push-based sync for steps, heart rate, sleep, and workouts. iOS 13+ required."
keywords: ["healthkit integration", "apple health api", "apple health data", "healthkit open source"]
"og:title": "HealthKit Integration with Healine | Apple Health"
...
"og:type": "article"
"twitter:title": "HealthKit Integration with Healine | Apple Health"
...
Apple Health integration uses a **push-based model** where health data is sent from the user's device to Healine using our mobile SDK. This is different from cloud providers like Garmin or Polar that use webhooks.
...
Even if a provider is **not yet directly supported** by Healine, its data can still be collected by syncing it into Apple Health first.
