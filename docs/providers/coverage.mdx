...
"og:title": "Wearable API Coverage Matrix | Healine"
...
"og:type": "article"
"twitter:title": "Wearable API Coverage Matrix | Healine"
...
<Accordion title="Polar">
  **Integration Method:** OAuth 2.0 (pull + webhook)
  ...
  - Webhooks are notify-only: Polar sends a lightweight event payload, Healine acknowledges immediately and enqueues a targeted sync for the affected data type
...
Healine normalizes all incoming data to consistent units and formats:
...
```bash
curl -X GET "http://localhost:8000/api/v1/users/{user_id}/timeseries\
