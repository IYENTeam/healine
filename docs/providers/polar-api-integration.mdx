...
"og:title": "Polar API Integration | Healine"
...
"og:type": "article"
"twitter:title": "Polar API Integration | Healine"
...
<Note>
**Need help with your Polar integration?** Pop into our [Discord](https://discord.gg/qrcfFnNE6H) if you have questions or want to discover how Healine can solve your problems.
</Note>
...
| Method | Description |
|--------|-------------|
| **Polling (pull) — Exercises** | Healine fetches exercises via the AccessLink v3 API (`/v3/exercises`). Returns all exercises available in Polar Flow for the connected user. |
...
| **Webhooks (push)** | Polar sends a lightweight notification on each event (exercise, sleep, activity, etc.). Healine acknowledges immediately and enqueues a targeted sync for the affected data type. |
...
```
  ...
  <Step title="Connect a user via OAuth">
    With credentials configured and your Healine instance running, initiate the OAuth flow to connect a user's Polar account.
    ...
    **3. Polar redirects back** to the callback URI configured in your `.env` (`POLAR_REDIRECT_URI`). Healine automatically exchanges the authorization code for access tokens and registers the user with the Polar API.
