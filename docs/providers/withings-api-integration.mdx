...
"og:title": "Withings API Integration | Healine"
...
"og:type": "article"
"twitter:title": "Withings API Integration | Healine"
...
<Note>
**Need help with your Withings integration?** Pop into our [Discord](https://discord.gg/qrcfFnNE6H) if you have questions or want to discover how Healine can solve your problems.
</Note>
...
Everything Healine currently imports from Withings, with the Withings API plan each item needs:
...
| Method | Description |
|--------|-------------|
| **Polling (pull)** - *default* | Healine fetches body measures, daily activity, sleep, and workouts via the Withings Measure and Sleep APIs. This is the default live-sync mode and needs no extra setup beyond OAuth. |
| **Webhooks (push)** - *opt-in* | Withings notifies Healine when new data is available for a user. Subscriptions are per-user (see below), so enabling webhooks requires a publicly reachable callback before any subscription can be created. |
...
<Steps>
  ...
  <Step title="Configure credentials in Healine">
...
```
  ...
  <Step title="Connect a user via OAuth">
    With credentials configured and your Healine instance running, initiate the OAuth flow.
    ...
    **3. Withings redirects back** to the callback URI derived from `API_BASE_URL` (`{API_BASE_URL}/api/v1/oauth/withings/callback`). Healine automatically exchanges the authorization code for access and refresh tokens.
  ...
  <Step title="Enable webhooks (optional)">
    Withings subscriptions are **per user**: each one is created with that user's own bearer token, so there's nothing to register at the application level. Instead, you switch Withings' live-sync mode to **Webhook**, and Healine subscribes every active Withings connection using its own token; any user who connects afterward is subscribed automatically at connect time.

    **Switch Withings to webhook mode from the dashboard:**

    In your Healine dashboard, go to **Settings → Providers**, find **Withings**, and switch its live-sync control from **Periodic pull** to **Webhook**.

    Healine derives the callback URL from your `API_BASE_URL` and `WITHINGS_WEBHOOK_TOKEN` as `{API_BASE_URL}/api/v1/providers/withings/webhooks?token={WITHINGS_WEBHOOK_TOKEN}`, and subscribes each user's connection to it via Withings' Notify API.
    ...
    **This is the single most likely thing to waste your afternoon in local development.** Withings enforces hard constraints on the callback URL and refuses to create a subscription otherwise:
    ...
    <Note>
    Switching back to **Periodic pull** revokes the per-user subscriptions Healine created; polling keeps working regardless of webhook state.
    </Note>
  </Step>
  ...
  <Step title="Verify the integration">
    Once data has synced, fetch it via the Healine API:
...
| `user.sleepevents` | Bed-sensor event notifications (bed in/out, inflate completion) - not requested by default; Healine doesn't subscribe to those notification categories |

<Note>
The scopes requested by Healine are controlled by `WITHINGS_DEFAULT_SCOPE`, a **comma-separated** list - see the warning in the configuration step above.
</Note>
...
Withings enforces the request limit once per registered application, not per user - calling an endpoint for two users counts as two requests against the same allowance. Healine does not pace calls against it: in pull mode every periodic sync fetches measures, activity, sleep and workouts for every connected user in one burst, and calls that hit 429 are retried a few times and then dropped. Beyond a few dozen users this will exhaust the allowance on every tick, so switch Withings to **webhook mode** (see **Enable webhooks (optional)** above) - then Healine only fetches data for users Withings has just notified about.
...
<CardGroup cols={2}>
  <Card title="API Reference" icon="terminal" href="/api-reference/introduction">
    Explore the Healine API endpoints.
  </Card>
...
- Check [Healine issues](https://github.com/IYENTeam/healine/issues).
