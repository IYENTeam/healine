...
"og:title": "Google Health API Integration | Healine"
...
"og:type": "article"
"twitter:title": "Google Health API Integration | Healine"
"twitter:description": "Connect Google Health via OAuth 2.0. Reconcile or list fetch modes; notify-only webhooks."
"twitter:card": "summary_large_image"
---

<Note>
**Need help with your Google Health integration?** Pop into our [Discord](https://discord.gg/qrcfFnNE6H) if you have questions or want to discover how Healine can solve your problems.
</Note>

## Overview

Google Health data reaches Healine through two independent paths, each its own provider:
...
| Method | Description |
|--------|-------------|
| **Polling (pull)** | Healine periodically fetches data via the Health API (Celery Beat), driven by the [fetch mode](#data-granularity-and-fetch-modes) below. |
| **Webhooks (notify-only)** | Google sends a lightweight ping naming the changed `dataType` and time intervals; Healine then fetches the changed data over REST. Requires a project-level [subscriber registration](#webhook-subscriber-registration). |

## Data granularity and fetch modes

The Health API exposes the same underlying data through three different operations. Healine picks one per the `DEFAULT_DATA_GRANULARITY` setting, and — at the finest granularity — the `GOOGLE_USE_RECONCILE` flag.
...
- **`list`** returns the **raw per-source points**, each tagged with its originating device. This preserves device attribution but stores overlapping sources separately; Healine then **deduplicates on read** by source priority (it does *not* sum them).
...
<Steps>
  <Step title="Enable the Health API and create an OAuth client">
    ...
    Then set `GOOGLE_LEGACY_OAUTH_PATH=false` so Healine sends that URI. New integrations should always use this path.
...
```
  ...
  <Step title="Connect a user via OAuth">
    ...
    <Tip>
    **Using the Healine frontend?** You don't need any of the curls below — open the connect view and click **Connect** on Google Health. The frontend runs this whole authorize → consent → callback → verify flow for you. The steps below are for integrating directly against the API.
    </Tip>
    ...
    **3. Google redirects back** to `{API_BASE_URL}/api/v1/oauth/google_health/callback`; Healine exchanges the code for tokens and stores the connection. It also resolves the user's stable `healthUserId` (via the Health API identity endpoint) as the connection's provider user id — this is what inbound webhooks are matched against.
...
Google Health webhooks are **notify-only**: each notification names the changed `dataType`, `operation` (`UPSERT`/`DELETE`), and the physical-time `intervals` that changed — but carries no data. Healine verifies the request, then fetches the changed data over REST for those intervals.

Two distinct secrets are involved:

- **`GOOGLE_WEBHOOK_SECRET`** — a bearer token you register with Google that it echoes back in the `Authorization` header of every notification, so Healine can verify the ping is genuine. Defaults to `SECRET_KEY` if unset.
...
<Warning>
`gcloud` commands need the project **ID** (e.g. `healine-prod`). The `GOOGLE_PROJECT_ID` env var, however, needs the project **number** (e.g. `123456789012`) — the subscriber API requires the number in its path.
</Warning>
...
notification fails signature verification and Healine returns 401.
</Warning>
