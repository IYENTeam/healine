...
description: "How Healine handles one provider account (e.g. Garmin) connected to multiple Healine user profiles: webhook fan-out, shared sync locks, and backfill coordination."
keywords: ["multi-account sync", "shared garmin account", "linked accounts", "webhook fan-out", "sync deduplication"]
"og:title": "Multi-Account Sync | Healine Architecture"
"og:description": "One provider account linked to multiple Healine profiles: webhook fan-out, shared sync locks, and backfill coordination without duplicate API calls."
"og:type": "article"
"twitter:title": "Multi-Account Sync | Healine Architecture"
"twitter:description": "One provider account linked to multiple Healine profiles: webhook fan-out, shared sync locks, and backfill coordination without duplicate API calls."
"twitter:card": "summary_large_image"
---

## Overview

Some providers (Garmin, Whoop, Oura) identify users by a stable external ID (`provider_user_id`) stored in `user_connection`. This allows one physical provider account to be connected to multiple Healine profiles — a common pattern for testing environments or shared devices.
...
When multiple Healine profiles share the same provider account, the platform must avoid:
...
The Connections page shows an amber **"N linked"** chip on any active connection whose `provider_user_id` is shared with at least one other Healine profile. Hovering the chip shows a tooltip with shortened UUIDs linking to the other profiles.
...
- **Webhook** — the provider pushes data to us. The payload arrives once regardless of how many Healine profiles share the account, so there is nothing to deduplicate at the API level. We fan out the single payload to all profiles internally.
...
1. `get_all_by_provider_user_id("garmin", garmin_user_id)` returns all active Healine connections sharing that Garmin account, ordered by `created_at asc` so the oldest profile is always index 0.
...
Garmin backfill works by making one API request per data type per time window, then waiting for Garmin to push the result via webhook. Because Garmin prohibits concurrent backfill requests from the same account, only **one** Healine profile may run the backfill chain.
