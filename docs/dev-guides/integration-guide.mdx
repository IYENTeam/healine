---
title: "Healine Integration Guide"
...
keywords: ["health api integration guide", "wearable api integration", "healine api tutorial", "health data api tutorial"]
"og:title": "Healine Integration Guide"
"og:description": "Create users, connect providers via OAuth, fetch health metrics. FastAPI examples. Any backend language."
"og:type": "article"
"twitter:title": "Healine Integration Guide"
...
This guide walks you through the complete integration flow with Healine - creating users from your backend, handing users off to wearable providers via OAuth (the frontend redirect dance included), and fetching normalized health data back.

## Prerequisites

Before you begin, ensure you have:

- Healine instance running (self-hosted or cloud)
- API Key generated from the settings tab in Healine Developer portal
...
<Tabs>
  <Tab title="When to Use">
    ...
    | **SDK Token** | Mobile apps pushing health data to Healine |
  </Tab>
...
When a user registers in your application, create a corresponding user in Healine.
...
| `first_name` | `string` | Optional | Max 100 characters. Useful for display in the Healine dashboard.                      |
| `last_name`  | `string` | Optional | Max 100 characters. Useful for display in the Healine dashboard.                      |
...
<Warning>
**Store the Healine User ID!** You'll need this ID for all subsequent API calls.
</Warning>

### Update Your User Model

Add a field to store the Healine user ID:
...
class User(Base):
    ...
    email: Mapped[str]
    healine_user_id: Mapped[UUID | None]  # Add this!
```
...
<Note>
Healine does **not** enforce uniqueness on `email`, `first_name`, or `last_name` - calling `POST /api/v1/users` twice with the same email will happily create two separate user records. Your backend is the source of truth for user identity and is responsible for ensuring you only create one Healine user per user in your system (e.g. by storing `healine_user_id` on your own user row and checking it before creating).
</Note>

<Warning>
The legacy `external_user_id` column does still carry a DB-level unique constraint, so sending a duplicate value there will fail with an integrity error. **The field is deprecated** and no data-fetching endpoint accepts it - do not rely on it for deduplication. Use the pattern above (store the Healine UUID on your side) instead.
</Warning>
...
<Steps>
  ...
  <Step title="Your backend calls the authorize endpoint">
    Request the authorization URL from Healine.
  </Step>
  <Step title="Redirect user to authorization_url">
    The user authenticates with their wearable provider.
  </Step>
  <Step title="Provider redirects to Healine callback">
    Healine handles the OAuth callback automatically.
  </Step>
  <Step title="Healine redirects to your app">
...
You do **not** need to explicitly trigger a sync after OAuth completes. On the first successful provider connection, Healine automatically dispatches a historical backfill:
...
Here's a complete Python client class for Healine integration:
...
class HealineClient:
    ...
    async def get_or_create_user(self, local_user) -> dict:
        """Create an Healine user if we haven't already, otherwise reuse the stored ID.

        `local_user` is your own user row - it stores `healine_user_id` once set
        so we never create a second Healine user for the same person.
        """
        async with httpx.AsyncClient() as client:
            if local_user.healine_user_id:
                resp = await client.get(
                    f"{self.base_url}/api/v1/users/{local_user.healine_user_id}",
            ...
            user = resp.json()
            local_user.healine_user_id = user["id"]  # persist on your side
            return user
...
<AccordionGroup>
  <Accordion title="Docker container can't reach Healine on localhost">
    If your app runs in Docker and Healine runs on the host machine:
  ...
  <Accordion title="Multiple users created for the same email">
    `email` has no unique constraint in Healine - calling `POST /api/v1/users` twice with the same email will create two separate records. Your backend is the source of truth for user identity, so store the Healine `id` (UUID) on your own user row the first time you create it, and reuse that ID for every subsequent call:

    ```python
    if not local_user.healine_user_id:
        resp = await client.post("/api/v1/users", json={"email": local_user.email})
        local_user.healine_user_id = resp.json()["id"]
        # persist local_user
    ```
...
<CardGroup cols={2}>
  ...
  <Card title="GitHub" icon="github" href="https://github.com/IYENTeam/healine">
