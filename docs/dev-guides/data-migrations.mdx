...
keywords: ["data migration", "backfill", "database", "healine"]
"og:title": "Data Migrations | Healine"
...
The practical consequence: if you upgrade Healine long after a migration shipped, the corresponding script may already have been removed from `app.sh` and **will not run automatically**. In that case, run it manually (see below). Scripts that were never added to `app.sh` must always be run manually.
