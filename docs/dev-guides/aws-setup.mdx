...
keywords: ["aws s3 apple health import", "aws sns health data", "healine aws setup", "s3 presigned url health xml"]
"og:title": "AWS S3 Setup for Apple Health XML Imports | Healine"
...
"og:type": "article"
"twitter:title": "AWS S3 Setup for Apple Health XML Imports | Healine"
...
<Steps>
  ...
  <Step title="Create Bucket">
    ...
    - **Bucket name**: Choose a unique name (e.g., `healine-xml`)
...
```json
{
  ...
  "Condition": {
    "ArnLike": {
      "aws:SourceArn": "arn:aws:s3:::healine-xml"
    }
  }
}
```
...
<Warning>
  Replace `123456789012` with your AWS Account ID, `healine-xml` with your bucket name, and `owear` with your topic name.
</Warning>
