...
keywords: ["raw payload storage wearables", "wearable data debug", "health api payload capture", "healine debug"]
"og:title": "Raw Payload Storage | Healine Debug Tool"
...
"og:type": "article"
"twitter:title": "Raw Payload Storage | Healine Debug Tool"
...
<Tabs>
  ...
  <Tab title="Development (MinIO)">
    ...
    **1. Start MinIO** (on the same Docker network as the app):
    ...
    <Warning>
      Use `--hostname minio` to register a clean DNS name in the Docker network. Boto3 rejects hostnames with special characters (e.g. double underscores), so container names like `minio__healine` won't work as endpoint URLs.
    </Warning>
    ...
    **2. Create the bucket** - open the MinIO Console at [http://localhost:9003](http://localhost:9003) (login `minioadmin` / `minioadmin`) and create a bucket called `raw-payloads`, or use the CLI:
    ...
    ```bash
    docker exec minio-healine mc alias set local http://localhost:9000 minioadmin minioadmin
    docker exec minio-healine mc mb local/raw-payloads
    ```
