<script lang="ts">
	...
	const Icon = $derived(workoutIcon(workout.type));
	const when = $derived(
		`${formatLocalDay(workout.start_time, workout.zone_offset)} · ${formatLocalTime(
			workout.start_time,
			workout.zone_offset
		)}`
	);
	...
	const device = $derived(workout.source.device_name ?? workout.source.device);
	const origin = $derived([providerLabel, device].filter(Boolean).join(' · '));
	const DeviceIcon = $derived(deviceIcon(workout.source.device_type));
</script>
...
>
	...
	<h3>
		...
		>
			...
			<span class="flex shrink-0 items-center gap-1.5" title={origin}>
				<ProviderMark provider={workout.source.provider} label={providerLabel} size="sm" />
	...
	{#if expanded}
		<div id={panelId}><WorkoutDetails {workout} {userId} {ondelete} /></div>
	{/if}
</article>
